...
// Function to load the list from the file into the array and increment the counter for each name.
void loadList(FILE *file, struct Person list[], int *count) {
    ...
    while (fscanf(file, "%6s: %[^\n]", status, buffer) == 2) {
        ...
        list[*count].name[sizeof(list[*count].name) - 1] = '\0'; // Null-terminate
        (*count)++;  // Increment the count directly
    }
} 
...
void saveListToFile(struct Person list[], int count) {
    FILE *file = fopen("Santas_List.txt", "w");
    if (file == NULL) {
