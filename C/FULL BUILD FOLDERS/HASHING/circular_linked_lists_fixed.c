...
Node* createNode(int value) {  // Handle integer values instead of strings
    Node* newNode = (Node*)malloc(sizeof(Node));
    newNode->data = value;  // Correct assignment of integer data
    newNode->next = NULL;
    return newNode;
}

void insertAtEnd(Node** head, int value) {  // Changed to handle integers
...
void sortCircularList(Node* head, int (*compare)(int, int)) {  // Compare integers
...
int compareIntegers(int a, int b) {  // Integer comparison
    return a - b;
}
